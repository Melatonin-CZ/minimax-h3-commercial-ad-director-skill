# MiniMax H3 Commercial Ad Prompt System — Index & Model Facts
...
- `VIDEO_PROMPT_WRITING_GUIDE_ref_en.md` — the full-reference-mode format (`subject_definitions`, `summary`, `retention_analysis`, `detailed_description`) for prompts built from reference images/video/audio.
...
| `commercial_ad_styles.md` | Six ad structure frameworks + a universal end-card convention, plus seven category visual playbooks, translated into reusable prompt vocabulary |
| `timeline_methodology.md` | How to plan a movement timeline at 0.1-second resolution — tuned for hook-precision and guaranteed CTA/end-card time — and compress it into valid prompt syntax |
| `certification_checklist.md` | The combined "certificate" checklist — what a prompt must satisfy before it's considered generation-ready |
| `worked_examples.md` | Complete prompts pairing a structure with a category playbook, each backed by a 0.1s timing chart |

Recommended reading order for a new prompt: `certification_checklist.md` (checklist) → `commercial_ad_styles.md` (pick a structure + a category) → `timeline_methodology.md` (build the timing chart) → `worked_examples.md` (see it done end to end) → write the prompt in the syntax from the base/ref guides.
